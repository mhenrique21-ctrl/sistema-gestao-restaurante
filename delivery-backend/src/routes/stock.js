...
// POST /api/stock/venda-externa — baixa de venda que aconteceu FORA do PDV.
// ============================================================================
// A cafeteria vende no Eclética Food, não no PDV, então a baixa automática do
// pedido (services/stock.js) nunca dispara para esses produtos. O App Gestão lê
// os XML de NFC-e e manda o total vendido por dia; aqui isso vira movimento.
//
// O PONTO DELICADO É IDEMPOTÊNCIA. A ponte do Eclética REENVIA o dia inteiro a
// cada 2 minutos (é assim que ela corrige um dia truncado). Se cada chamada
// descontasse o total do dia, um dia de venda zeraria o estoque sozinho ao
// longo da tarde. Por isso guardamos quanto já foi aplicado por
// (fonte, data, produto) e movimentamos só a DIFERENÇA.
//
// O razão continua append-only: corrigir para menos gera um movimento de
// estorno, não um UPDATE no movimento antigo — mexer em quantity depois de
// gravado corromperia o balance_after de todas as linhas seguintes.
let _tabelaVendaExternaPronta = null;
function garantirTabelaVendaExterna() {
  if (!_tabelaVendaExternaPronta) {
    _tabelaVendaExternaPronta = pool.query(`
      CREATE TABLE IF NOT EXISTS vendas_externas_estoque (
        fonte VARCHAR(40) NOT NULL,
        sale_date DATE NOT NULL,
        product_id UUID NOT NULL,
        quantidade NUMERIC(12,3) NOT NULL DEFAULT 0,
        updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        PRIMARY KEY (fonte, sale_date, product_id)
      )
    `).catch((e) => { _tabelaVendaExternaPronta = null; throw e; });
  }
  return _tabelaVendaExternaPronta;
}

// Mesma normalização que o Gestão usa (foldNome): sem acento, minúsculo,
// espaço colapsado. Feita em JS e não em SQL porque unaccent é extensão que
// pode não estar instalada — e o cardápio tem ~200 linhas, não é gargalo.
const fold = (s) => String(s || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '')
  .toLowerCase().replace(/\s+/g, ' ').trim();

router.post('/venda-externa', requireRole('admin'), async (req, res) => {
  try {
    const { fonte: fonteRaw, data, itens } = req.body || {};
    const fonte = String(fonteRaw || '').toLowerCase().replace(/[^a-z0-9_]/g, '').slice(0, 40);
    if (!fonte) return res.status(400).json({ error: 'Informe a fonte da venda (ex.: ecletica)' });
    if (!/^\d{4}-\d{2}-\d{2}$/.test(String(data || ''))) return res.status(400).json({ error: 'data inválida (use AAAA-MM-DD)' });
    if (!Array.isArray(itens)) return res.status(400).json({ error: 'itens deve ser uma lista' });

    await garantirTabelaVendaExterna();

    const cat = await pool.query('SELECT id, name, track_stock FROM products');
    const porNome = new Map();
    const porId = new Map();
    for (const p of cat.rows) { porNome.set(fold(p.name), p); porId.set(p.id, p); }

    const naoEncontrados = [];
    const semControle = [];
    // Agrega antes de aplicar: o mesmo produto pode chegar em duas linhas
    // (grafias diferentes que normalizam igual), e aplicar duas vezes na mesma
    // chave faria a segunda ver a primeira como "já aplicado" e zerar o delta.
    const alvo = new Map();
    for (const it of itens) {
      const qtd = Math.max(0, parseFloat(it?.quantidade ?? it?.qtd) || 0);
      const p = it?.product_id ? porId.get(it.product_id) : porNome.get(fold(it?.nome));
      if (!p) { naoEncontrados.push(String(it?.nome || it?.product_id || '?')); continue; }
      if (!p.track_stock) { semControle.push(p.name); continue; }
      alvo.set(p.id, { produto: p.name, qtd: (alvo.get(p.id)?.qtd || 0) + qtd });
    }

    const aplicados = [];
    for (const [productId, info] of alvo) {
      const ant = await pool.query(
        'SELECT quantidade FROM vendas_externas_estoque WHERE fonte = $1 AND sale_date = $2 AND product_id = $3',
        [fonte, data, productId]
      );
      const anterior = parseFloat(ant.rows[0]?.quantidade || 0);
      const delta = Math.round((info.qtd - anterior) * 1000) / 1000;
      if (delta !== 0) {
        await moverEstoque([{ product_id: productId, quantity: Math.abs(delta) }], {
          tipo: delta > 0 ? 'venda' : 'estorno',
          sinal: delta > 0 ? -1 : 1,
          motivo: `Venda ${fonte} ${data}`,
        });
      }
      await pool.query(
        `INSERT INTO vendas_externas_estoque (fonte, sale_date, product_id, quantidade)
         VALUES ($1, $2, $3, ${info.qtd})
         ON CONFLICT (fonte, sale_date, product_id)
         DO UPDATE SET quantidade = ${info.qtd}, updated_at = NOW()`,
        [fonte, data, productId]
      );
      aplicados.push({ produto: info.produto, vendido: info.qtd, movimentado: -delta });
    }

    res.json({ ok: true, data, fonte, aplicados, naoEncontrados, semControle });
  } catch (err) {
    return erro(res, err, '[stock/venda-externa]');
  }
});
