...
const dbCaso = {
  funcionarios: [FUNC],
  // A falta agora custa DOIS dias: o não trabalhado e o repouso da semana
  // (Lei 605/49, art. 6º). O valor sai de src/faltaClt.js a partir da data —
  // não do campo `desconto` gravado, que só cobria o dia.
  faltas: [{ funcionarioId: 'f1', mes: MES, data: '2026-09-14', dias: 1, tipo: 'injustificada' }],
...
test('falta desconta o dia E o DSR de quem faltou', () => {
  // Antes o total de faltas era calculado e NUNCA usado no valor a receber, e
  // mesmo depois de corrigido cobria só o dia — sem o repouso que a lei manda
  // descontar junto.
  const h = calcularHolerite(dbCaso, FUNC, MES);
  assert.equal(h.faltas, 133.33);         // 2 × (2000 ÷ 30)
  assert.equal(h.faltaDetalhe.dsrPerdidos, 1);
  assert.equal(h.descontos, 363.33);      // 133,33 + 80 + 150
  assert.equal(h.liquido, 1636.67);
});
...
test('bonificação não entra no líquido — ela sai pela conta de encargos', () => {
  ...
  assert.equal(e.folha, 1636.67);
  assert.equal(e.encargo, 200);           // 0 patronal + 200 bonificação
  assert.equal(e.holerite.aReceber, 1836.67);
});

test('a folha da DRE passa a ser o desembolso, sem nada inventado', () => {
  const e = contasEsperadas(dbCaso, FUNC, MES);
  assert.equal(e.total, 1836.67);
  assert.equal(e.holerite.custoEmpresa, 1836.67);
...
test('adiantamento desconta do holerite E soma no desembolso', () => {
  ...
  assert.equal(e.folha, 1136.67);         // 2000 − 133,33 − 80 − 150 − 500
  assert.equal(e.adiantamento, 500);
  assert.equal(e.folha + e.adiantamento, 1636.67);   // o mesmo líquido de antes
  assert.equal(e.total, 1836.67);         // desembolso não muda por adiantar
});
...
test('encargo patronal é custo da empresa e não toca no líquido', () => {
  ...
  const e = contasEsperadas(db, FUNC, MES);
  assert.equal(e.folha, 1636.67);         // patronal não desce do líquido
  assert.equal(e.encargo, 360);           // 160 patronal + 200 bonificação
  assert.equal(e.total, 1996.67);
});
...
test('desconto maior que o salário não vira conta negativa', () => {
  ...
  assert.equal(h.descontoNaoAbsorvido, 3363.33);
});
...
test('conta casa por id do funcionário, não pelo nome', () => {
  ...
  const db = comContas([
    { id: 'c1', funcionarioId: 'f1', mesRef: MES, tipoRh: 'folha', valor: 1636.67, tipo: 'saida' },
    { id: 'c2', fornecedor: 'Mário Henrique', valor: 999, tipo: 'saida' },
  ]);
  const l = contasLancadas(db, 'f1', MES);
  assert.equal(l.folha, 1636.67);
  assert.equal(l.total, 1636.67);
});
...
test('conferência acusa a folha lançada duas vezes', () => {
  ...
  const db = comContas([
    { id: 'c1', funcionarioId: 'f1', mesRef: MES, tipoRh: 'folha', valor: 1636.67, tipo: 'saida' },
    { id: 'c2', funcionarioId: 'f1', mesRef: MES, tipoRh: 'folha', valor: 1636.67, tipo: 'saida' },
    { id: 'c3', funcionarioId: 'f1', mesRef: MES, tipoRh: 'encargo', valor: 200, tipo: 'saida' },
  ]);
  const r = conciliarMes(db, MES);
  assert.equal(r.linhas[0].status, 'divergente');
  assert.equal(r.linhas[0].diferenca, 1636.67);
...
test('holerite fechado e nada lançado aparece como pendência', () => {
  ...
  assert.equal(r.totalEsperado, 1836.67);
...
test('contas antigas de falta e consumação ficam visíveis para o dono decidir', () => {
  ...
  const db = comContas([
    ...
    { id: 'v3', origem: 'folha_rh', categoria: 'Salários', tipo: 'saida', vencimento: `${MES}-05`, valor: 1636.67 },
  ]);
  const r = conciliarMes(db, MES);
